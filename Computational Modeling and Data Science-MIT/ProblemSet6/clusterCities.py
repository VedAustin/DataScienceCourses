...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        # TO DO
        distance = 10**6
        thisCluster = self.points
        thatCluster = other.points
        for thisPoint in thisCluster:
            for thatPoint in thatCluster:
                temp = thisPoint.distance(thatPoint)
                if temp < distance:
                    distance = temp
        return distance
    ...
    def averageLinkageDist(self, other):
        ...
        for thisPoint in thisCluster:
            for thatPoint in thatCluster:
                temp.append(thisPoint.distance(thatPoint))
...
class ClusterSet(object):
    ...
    def findClosest(self, linkage):
        ...
        distance = 10**6
        for (c1,c2) in itertools.combinations(self.members,2):
            dist = linkage.__call__(c1,c2)
            #print dist
            if dist < distance:
                closest = (c1,c2)
                distance = dist
...
def hCluster(points, linkage, numClusters, printHistory):
    ...
    while cS.numClusters() > numClusters:
        #print cS.numClusters(),numClusters
